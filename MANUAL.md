...
### Windows
* Скачайте архив из раздела [Releases](../../releases). Для Windows 7 используйте 32-битный файл с индексом `i386`, для Windows 10+ &mdash; 64-разрядный `x86_64`.
* Для Windows 7+ доступны варианты с аппаратным выводом видео `SDL2` (предпочтительный) и программный `Qt` (сильнее нагружает процессор). 
* Для Windows 10+ доступны варианты `SDL2` (аппаратный, стабильный), `Qt` (программный, сильнее грузит процессор), `opengl` (аппаратный, экспериментальный, предпочтительный).
...
* Запустите программу еще раз. При необходимости подтвердите разрешение в открывшемся окне.
...
Окно работает в обе стороны: клавиши подсвечиваются, когда их нажимают &ndash; с обычной клавиатуры, из сценария или снаружи через MCP, &ndash; а щелчок мышью по нарисованной клавише отправляет ее в машину. Нажимается именно та клавиша, которая нарисована, поэтому щелчок по букве дает букву независимо от того, в какой раскладке стоит клавиатура компьютера.

Клавиши-защелки (РУС и ЛАТ, ЗАГЛ и СТР у БК) удерживать не нужно: машина хранит их состояние сама, и на рисунке подсвечена та из пары, которая включена сейчас. Клавиши-модификаторы, которые на настоящей машине держат нажатыми (у БК это клавиша нижнего регистра со стрелкой вниз, СУ и АР2), залипают по щелчку и отпускаются повторным: у мыши и у пальца одна точка касания, двух клавиш сразу не нажать. Пока модификатор нажат, он подсвечен. С клавиатуры компьютера у БК нижний регистр, СУ и АР2 нажимаются клавишами Shift, Ctrl и Alt.

Окно тянется за край: рисунок векторный, поэтому остается резким на любом размере, а пропорции
клавиатуры окно держит само. Выбранная ширина запоминается до следующего запуска.

Щелчки записываются в сценарий наравне с остальными действиями.
