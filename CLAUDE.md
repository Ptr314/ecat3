...
Devices expose themselves to scripts through `get_device_fields()` / `get_field()` (readable state) and `get_device_commands()` / `send_command()` (actions), declared in `ComputerDevice` and overridden down the hierarchy. `AddressableDevice` already provides `value`/`size` plus `set`/`fill`, and `CPU` provides `pc`/`registers`/`flags` plus `stop`/`run`/`step`/`breakpoint`/`setreg`, so a new device usually needs nothing to be scriptable.
...
- Hex prefix: `$` (e.g., `$FF`), Binary: `#` (e.g., `#1010`), Multiplier: `k` (e.g., `64k`)
