...
`qtsvg` собирается по умолчанию: он нужен экранной клавиатуре, и скрипт выпуска кладет `Qt5Svg.dll` рядом с программой. Вызова `QSvgRenderer::transformForElement` в Qt 5.6 еще нет, клавиатура берет вместо него `matrixForElement` с той же цепочкой преобразований родителей.
...
Сборка для браузера не использует Qt: в неё идёт только Qt-независимое
...
Эмулятор собран с потоками (`-pthread`), а`SharedArrayBuffer` доступен 
только на изолированном происхождении, поэтому обязательна установка следующих заголовков:
...
Готовый пример настройки nginx с пояснениями лежит в самом пакете
