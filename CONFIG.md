...
* Обращение по адресу, на котором не отображено ни одно устройство, заканчивается таймаутом магистрали: текущая команда прерывается и происходит прерывание по вектору 004. ПЗУ БК так определяют, какие блоки памяти действительно установлены, поэтому дырки в карте памяти должны оставаться дырками. Диапазон, отображенный только на чтение, отвечает и на запись &ndash; она просто игнорируется, как в реальном ПЗУ.
