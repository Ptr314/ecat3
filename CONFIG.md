...
* Интерфейсы
    * __~congig__{?, in}: Может использоваться вместо параметра __congig__ для более гибкого подключения управляющих устройств. 
...
~~~
Параметры:
* __map__: Файл с расладкой клавиатуры (см. ниже).
...
Модификаторы используются, когда состояние Shift на эмулируемом компьютере не совпадает с нужным для ввода символа на хосте. Ноапример, при вводе &laquo;:&raquo; Shift нажимается, но на эмулируемой машине этот исмвол вводится без Shift &ndash; тогда в раскладке на нужной позиции будет &laquo;:_&raquo;, т.е. эта позиция в матрице соответствует двоеточию, но Shift нужно отпустить.
