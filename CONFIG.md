...
* Текстовое описание конфигурации должно находиться в файле, имя которого совпадает с файлом конфигурации, но с раширением __.md__ в формате [Markdown](https://ru.wikipedia.org/wiki/Markdown).
...
* Если ширина интерфейса у двух устройств различна, нужно указывать диапазон подключаемыъ битов.
...
* __type*__: Идентификатор типа компьютера. Используется для сбора конфиггурации в группы в диалоге выбора.
...
* __charmap*__: Файл с мэппингом кодировки на UTF-8. Файл должен состоять из 256 символов UTF-8, где номер символа &ndash; его код в исходном компьютере, а значение &ndash; выводится на экран.
    * Таблица используется в окнах дампов и отладчике. 
* __screenratio__: Соотношение сторон пикселя при выводе на экран. Используется, чтобы подогнать экран под соотношение 4:3 независимо от разрешения эмулиреумого компьютера.
...
* Основные параметры:
    ...
    * __config__: Устройство (порт), содержащее регистр конфигурации мэппера. Значения этого регистра используются в записях ___@memory___.
* Мэппинг памяти. Поиск подходящего условия происходи сверху вниз, поэтому первые строки имеют приоритет. Примеры записи:
...
* Мэппинг портов (при отключенном мэппинге на память).
...
* __clock*__: Частота импульсов, Гц. 
...
__Примечание__: В связи с особенносятми эмуляции, минимально возможная длина импульса фактически будет равна длине команды CPU, на которую он попадет.
