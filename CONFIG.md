...
* [Устройства](#Устройства)
    ...
    * [CPU](#CPU)
        ...
        * [i8080](#i8080)
        * [Z80](#Z80)
    * [Базовые устройства](#Базовые_устройства) 
        * [ram](#ram) (Блок ОЗУ)
        * [rom](#rom) (Блок ПЗУ)
    ...
    * [Машино-зависимые устройства](#Машино-зависимые-устройства)
        ...
        * [orion-128-display](#agat-display) (Дисплей Орион-128)
...
* __image*__: Имя файла образа:
    ...
    * Файл должен находиться в одном из следущих мест: 
...
* Параметры:
    ...
    * __mask__: Маска для записываемых значений. Совместно с __default__ позволяет задать неизменяемые биты. По умолчанию &ndash; $FFFFFFFF.
...
### register
Регистр-защелка или буфер. Запоминает входное значение по стробу на входе записи или при транслирует входы на выходы.
~~~
latch : register {
    type = latch-pos
	~in[0-3] = port-user.B[0-3]
...
* Параметры:
    * __type__: Тип регистра:
        * __buffer__: Входы транслируются на выходы.
        * __latch-pos__: Запись по положительному фронту на входе __c__.
        * По умолчанию &ndash; __latch-pos__;
...
### map-keyboard
Клавиатура, записывающа код клавиши в регистр. Соответствие клавиш и полученного кода задается во внешнем файле. 
~~~
...
* __port-ruslat__: Устройство-порт, в старший бит которого записывается статус раскладки.
...
Универсальный дисплейны контроллер на основе i8275. Поддерживаются различнве варианты генерации цвета.
