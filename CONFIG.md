...
* Интерфейсы:
    * __value{=size=, out}__: Выходные данные.
    * __access{1, out}__: Выдает импульс отрицательной полярности в момент записи значения.
    * __flip{1, in}__: При отрицательном фронте по этому интерфейсу происходит инверсия значения порта по маске __flipmask__. 
