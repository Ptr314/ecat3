...
с фиксированной палитрой из четырех цветов). На реальной машине режим выбирается
...
* Параметры
    * __divider__: Число тактов системной частоты на один тик таймера. По умолчанию &ndash; 128, что при 3 МГц дает шаг около 42.9 мкс.
...
| 1 | Непрерывный счет: после перехода через ноль счет продолжается |
| 2 | Разрешение индикации: при переходе через ноль устанавливается разряд 7 |
| 3 | Запрет повтора: счетчик не перезагружается из регистра уставки |
| 4 | Разрешение счета |
| 7 | Признак перехода через ноль (только чтение, сбрасывается записью в регистр) |

Запись в регистр управления запускает таймер и загружает счетчик из регистра
уставки. Типовое использование &ndash; отсчет заданного интервала с ожиданием
признака:
