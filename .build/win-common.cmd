...
REM   call win-common.cmd version <cmakelists> -- project version lands in %_VERSION%
...
REM ---------------------------------------------------------------------------
REM The version is read from the project() call in src/CMakeLists.txt, which is
REM the single source of truth. globals.h carries the same number but is
REM generated by configure_file(), so it is only in sync after a cmake run.
:version
set "_VERSION="
for /f "tokens=3" %%V in ('findstr /R /C:"^project(.* VERSION " "%~2"') do set "_VERSION=%%V"
