...
private:
    ...
    Interface i_int;
    Interface i_m1;
    //Полный адрес обращения к порту. У Z80 во время IN/OUT на старших
    //разрядах адреса стоит B (или A для команд с непосредственным номером
    //порта), и машины этим пользуются: Арго держит там номер столбца
    //клавиатуры, а сам порт декодирует только младший байт
    Interface i_io_address;
