...
def main():
    ...
    # Без Pillow compare_png() не может сосчитать точки и объявляет расхождением
    # любую разницу байтов. Допуск на мигающие курсор и строку меню тогда не
    # действует, и снимки расходятся то в одних тестах, то в других - похоже на
    # нестабильный эмулятор, хотя он ни при чем
    try:
        import PIL  # noqa: F401
    except ImportError:
        print("ВНИМАНИЕ: в этом Python нет Pillow (pip install pillow). Снимки сравниваются\n"
              "побайтно, без допуска на мигание, и расходятся от прогона к прогону.\n")
